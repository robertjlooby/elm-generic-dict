...
tests =
    let
        buildTests =
            suite "build Tests"
                [ test "empty"
                    <| assertEqual (GenericDict.fromList compare [])
                        (GenericDict.empty compare)
                , test "singleton"
                    <| assertEqual (GenericDict.fromList compare [ ( "k", "v" ) ])
                        (GenericDict.singleton compare "k" "v")
                , test "insert"
                    <| assertEqual (GenericDict.fromList compare [ ( "k", "v" ) ])
                        (GenericDict.insert "k" "v" <| GenericDict.empty compare)
                , test "insert replace"
                    <| assertEqual (GenericDict.fromList compare [ ( "k", "vv" ) ])
                ...
                , test "update Nothing"
                    <| assertEqual (GenericDict.empty compare)
                ...
                , test "remove not found"
                    <| assertEqual (GenericDict.singleton compare "k" "v")
            ...
            in
                suite "combine Tests"
                    [ test "union uses first groups comparer (and values in a collision)"
                        <| assertEqual [ ( 5, "ee" ), ( 4, "d" ), ( 3, "c" ), ( 2, "b" ), ( 1, "a" ) ]
        ...
        transformTests =
            suite "transform Tests"
                [ test "map"
                    <| assertEqual [ ( "Jerry", "the mouse" ), ( "Tom", "the cat" ) ]
                        (GenericDict.map (\k v -> "the " ++ v) animals |> GenericDict.toList)
                , test "foldl"
                    <| assertEqual "xJerrymouseTomcat"
                        (GenericDict.foldl (\k v acc -> acc ++ k ++ v) "x" animals)
                , test "foldr"
                    <| assertEqual "xTomcatJerrymouse"
                        (GenericDict.foldr (\k v acc -> acc ++ k ++ v) "x" animals)
                , test "filter"
                    <| assertEqual (GenericDict.singleton compare "Tom" "cat")
                        (GenericDict.filter (\k v -> k == "Tom") animals)
                , test "partition"
                    <| assertEqual ( GenericDict.singleton compare "Tom" "cat", GenericDict.singleton compare "Jerry" "mouse" )
                        (GenericDict.partition (\k v -> k == "Tom") animals)
                ]
    in
        suite "GenericDict Tests"
            [ buildTests
            , queryTests
            , combineTests
            , keyValueTests
