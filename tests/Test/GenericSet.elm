  ...
  let
    ...
    modifierTests =
      suite
        ...
        , test
            "insert does nothing if a value exists"
            <| assertEqual
                [ 1, 2, 3 ]
                (GenericSet.fromList compare [1..3]
                  |> GenericSet.insert 2
        ...
        , test
            "remove does nothing if a value does not exist"
            <| assertEqual
                [ 1, 2, 3 ]
                (GenericSet.fromList compare [1..3]
                  |> GenericSet.remove 5
    ...
    queryTests =
      suite
        ...
        , test
            "member returns True if set includes value"
            <| assertEqual True (GenericSet.member 50 set)
        , test
            "member returns False if set does not include value"
            <| assertEqual False (GenericSet.member 0 set)
        , test
            "member uses the comparer to determine equality"
            <| let
                comparer a a' =
                  compare a.id a'.id
    ...
    combiningTests =
      suite
        ...
        , test
            "union uses the comparer from the first set"
            <| let
                set1 =
        ...
        , test
            "intersect gets the overlap of two sets"
            <| let
                set1 =
        ...
        , test
            "intersect uses the comparer from the first set"
            <| let
                set1 =
                  GenericSet.fromList compare' [ 3, 2, 1 ]
        ...
        , test
            "diff gets the values in the first set that are not in the second"
            <| let
                set1 =
